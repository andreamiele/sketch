...
sketch=load_sketch_data("/home/ml4science2023/GAN/sketch/quickdraw_dataset/The Eiffel Tower.npz", 'train')
sketch2=load_sketch_data("/home/ml4science2023/GAN/sketch/quickdraw_dataset/church.npz", 'train')
sketch3=load_sketch_data("/home/ml4science2023/GAN/sketch/quickdraw_dataset/skull.npz", 'train')
sketch4=load_sketch_data("/home/ml4science2023/GAN/sketch/quickdraw_dataset/sun.npz", 'train')
...
class SketchEmbedding(nn.Module):
    ...
    def forward(self, sketch_tokens):
        ...
        token_embeddings = token_embeddings.squeeze(2)  # Remove the dimension with size 1
        token_embeddings = token_embeddings.transpose(0, 1)  # Transpose to get [seq_len, batch_size, d_model]
...
class Sketchformer(nn.Module):
    ...
    def forward(self, src, tgt, src_mask=None, tgt_mask=None, memory_mask=None,
      ...
      PAD_IDX = 0
      # Create a source and target key padding mask with shape [batch_size, sequence_length]
      src_key_padding_mask = (src == PAD_IDX)  # Transpose to get shape [batch_size, seq_len]
      tgt_key_padding_mask = (tgt == PAD_IDX)  # Transpose to get shape [batch_size, seq_len]
      ...
      # Ensure src_emb and tgt_emb are 3D tensors with correct shape [sequence_length, batch_size, embedding_dimension]
      src_emb = src_emb.squeeze(2).transpose(0, 1)  # Remove extra dimension and transpose
      tgt_emb = tgt_emb.squeeze(2).transpose(0, 1)  # Remove extra dimension and transpose
...
for epoch in range(num_epochs):
    ...
    for i, (sketches, _) in enumerate(dataloader):
        sketches = sketches.squeeze(2) 
        sketches = sketches.to(device)
        sos_tensor = torch.full((sketches.size(0), 1), SOS_token, dtype=torch.long, device=device)
    ...
    # Correct key padding masks to have shape [batch_size, sequence_length]
        src_key_padding_mask = (sketches == PAD_IDX).transpose(0,1)
        tgt_key_padding_mask = (tgt_input == PAD_IDX).transpose(0,1)

    # Create a causal mask for the target
        sequence_length = tgt_input.size(1)
        upper_triangular_mask = torch.triu(torch.ones((sequence_length, sequence_length), device=device), diagonal=1).bool()
        tgt_mask = tgt_mask & upper_triangular_mask

    # Forward pass with masks
        outputs = model(sketches, tgt_input, src_key_padding_mask=src_key_padding_mask, tgt_key_padding_mask=tgt_key_padding_mask)
    ...
    model_save_path = os.path.join('saved_models', f'model_epoch_{epoch+1}.pth')
